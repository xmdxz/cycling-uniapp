			...
			<u-sticky :enable="stickyEnable">
				<view class="stickt">
					<view class="search">
						...
						<u-search class="" placeholder="请输入要搜索的动态" disabled :show-action="false" @click="toSearch()"></u-search>
						...
						<view class="swiper-div">
							<view class="content" style="margin-bottom: 100upx;">
								<attentionitem
									...
									:avatar="item.avatar"
									:contentuser="item.username"
									:contentlevel="item.level"
									:contenttext="item.content"
									:key="index"
									contenttime="发布时间:2021年7月28号13时40分"
									:imgs="item.imgs"
									:id="item.id"
								></attentionitem>
							</view>
				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;margin-top: 20upx;" @scrolltolower="onreachBottom">
						<view class="swiper-div">
							...
							<!-- 公告 -->
							<view class="u-m-t-20"><u-notice-bar mode="horizontal" :list="textList"></u-notice-bar></view>
							<!-- 热门话题 -->
								...
								<view class="wrap" style="background: rgb(249,250,251);">
									<u-waterfall v-model="flowList" ref="uWaterfall">
										...
										<template v-slot:left="{ leftList }">
											<view class="water" v-for="(item, index) in leftList" :key="index" @click="toDynamicDetail">
												<!-- 文章图片 -->
												...
												<view class="content-info">
													...
													<view class="content-author">
														<u-avatar :src="item.avatar" size="40" style="vertical-align: middle;"></u-avatar>
														{{ item.author }}
													</view>
													<!-- 点赞数 -->
													<view class="content-like">
														<u-icon name="heart" color="rgb(109,111,111)" style="vertical-align: middle;" size="30"></u-icon>
														<text style="line-height: 1.8;">{{ item.like }}</text>
													</view>
												</view>
											</view>
										</template>
										...
										<template v-slot:right="{ rightList }">
											<view class="water" v-for="(item, index) in rightList" :key="index" @click="toDynamicDetail()">
												<!-- 文章图片 -->
												<u-lazy-load threshold="-450" border-radius="10" :image="item.image" :index="index" style="width: 100%;"></u-lazy-load>
												<!-- 文章标题 -->
												<view class="content-title">{{ item.title }}</view>
												<view class="content-info">
													<!-- 作者 -->
				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="width: 100%;margin-top: 20upx;margin-bottom: 100upx;height: 100vh;" @scrolltolower="addRandomDataTwo()">
						...
						<view class="u-margin-30 recommed">
							...
							<!-- 加载更多 -->
							<u-loadmore bg-color="rgb(240, 240, 240)" :status="loadStatusTwo" @loadmore="addRandomDataTwo()"></u-loadmore>
						</view>
					</scroll-view>
				</swiper-item>
			</swiper>
...
export default {
	...
	onLoad() {
		...
		let userString = decodeURIComponent(escape(window.atob(this.$store.state.vuex_token.split('.')[1])));
