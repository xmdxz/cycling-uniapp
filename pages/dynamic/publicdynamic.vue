<template>
	<view>
		...
		<view class="public-content">
			...
			<view class="tags">
				<u-field type="text" label="话题" :placeholder="topicPlaceholder" disabled right-icon="arrow-right"
					icon="attach" input-align="right" @click="showTopic()"></u-field>
				<u-field v-model="position" type="text" label="地点" :placeholder="regionPlaceholder"
					@click="showRegion()" disabled right-icon="arrow-right" icon="map" input-align="right"></u-field>
			</view>
			<view class="btn-pub">
			...
			<u-popup v-model="topicShow" mode="center" width="550rpx" height="700rpx">
				<view class="topic">
					<view class="topic-search">
						<u-search placeholder="请输入搜索的话题" v-model="searchTopic" :show-action="false"
							@change="searchTopics()"></u-search>
					</view>
					<view class="topic-content">
						...
						<view class="topic-item" v-for="item in topicList" :key="item.id">
							<u-tag :text="item.topicName" type="info" mode="light" shape="circle"
								@click="addTopicSelected(item)"></u-tag>
						</view>
					...
					<view class="topic-selected">
						<view class="topic-item" v-for="item in topicSelectedList">
							<u-tag :text="item.topicName" type="error" mode="light" shape="squire" :closeable="true"
								@close="deleteTopicSelected(item)"></u-tag>
						</view>
					</view>
				</view>
			</u-popup>
		...
		<u-picker mode="region" v-model="regionShow" :safe-area-inset-bottom="true" @confirm="getRegionData"
			:default-region="regionSelected"></u-picker>
	</view>
</template>
			...
			async public() {
				...
				if (this.$refs.uUpload.lists.length > 0) {
					...
					let res = await this.$u.api.publicDynamic({
						...
						position: this.regionSelected[0] +
							"-" +
							this.regionSelected[1] +
							"-" +
							this.regionSelected[2],
						topicId: topicSelectedIdList,
						imgName: JSON.parse(JSON.stringify(this.imgPushList))
					});
					console.log(res);
					if (res.code === 200) {
						setTimeout(function() {
					...
					qiniu.compressImage(currentFile, options).then((data) => {
						...
						const subscription = observable.subscribe({
							...
							complete: (res) => {
								...
								// 判断文件是否已上传完成
								if(files.length==this.imgPushList.length){
									//处理话题id
									let topicSelectedIdList = [];
									// 循环将选中的id放入新定义的数组
									for (let i = 0; i < this.topicSelectedList.length; i++) {
