<template>
	<view>
		<u-sticky><u-tabs ref="tabs" :list="list" :current="current" :is-scroll="false" :item-width="windowWidth" @change="changeTabs"></u-tabs></u-sticky>
		<view v-if="current == 0">
			...
			<u-card v-for="(item, index) in focused.list" :key="index" margin="1rpx 1rpx 0rpx 0rpx" :show-head="false" :show-foot="false" :border="false">
				<view slot="body">
					<view class="u-body-item u-flex u-border-bottom u-col-between u-p-t-0 card">
						...
						<u-button
							class="attention"
							:type="item.mutualConcern ? 'success' : 'default'"
							shape="circle"
							size="default"
							@click="focusedOperation(index, item.mutualConcern)"
						>
			...
			<view class="more"><u-loadmore @loadmore="loadmore(0)" :status="focused.more" /></view>
		</view>
		<view v-if="current == 1">
				...
				<view slot="body">
					<view class="u-body-item u-flex u-border-bottom u-col-between u-p-t-0 card">
						...
						<u-button
							...
							shape="circle"
							size="default"
							@click="fansOperation(index, item.relatedUserId, item.mutualConcern)"
						>
			...
			<view class="more"><u-loadmore @loadmore="loadmore(1)" :status="fans.more" /></view>
		</view>
...
export default {
	...
	watch: {
		...
		'focused.list': {
			handler(e) {
				if(e.length === 0){
					this.focused.show = true
				}else{
					this.focused.show = false
				}
			}
		}
	},
			...
			if (mutualConcern) {
				...
				uni.showModal({
					...
					success(res) {
						if (res.confirm) {
							uni.showLoading();
							that.$u.api.cancelFocused({
								focusedUserId:instead[index].relatedUserId
							}).then(result => {
								if (result > 0) {
									instead[index].mutualConcern = false;
									let remove = that.focused.list.findIndex(e => {
										return e.relatedUserId === instead[index].relatedUserId;
									});
									if (remove != -1) {
										that.focused.list.splice(remove, 1);
									}
									uni.hideLoading();
								} else {
									uni.hideLoading();
		...
		async focusedOperation(index, userId, mutualConcern) {
			...
			uni.showModal({
				...
				success(res) {
					if (res.confirm) {
						uni.showLoading();
						that.$u.api.cancelFocused({id:instead[index].id}).then(result => {
							if (result > 0) {
								let remove = that.fans.list.findIndex(e => {
									return e.relatedUserId === instead[index].relatedUserId;
								});
								if (remove != -1) {
									that.fans.list[remove].mutualConcern = false;
								}
