		...
		<view class="box-1" id="list-box">
			<view class="talk-list">
				<view v-for="(item, index) in talkList" :key="index" :id="`msg-${item.id}`">
					<view class="item flex_col" :class="item.sender == id ? 'push' : 'pull'">
						<image :src="item.sender == id ? senderPic : receiverPic" mode="aspectFill" class="pic"></image>
	...
	methods: {
		...
		async joinHistoryMsg() {
			...
			let join = async function() {
				...
				//通过当前页码及页数，模拟数据内容
				let result = await that.$u.api.getChatHistory({
					page: that.ajax.page,
					num: that.ajax.rows,
					receiveId: that.ajax.receiver
				});
				if(result > 0){
		...
		send() {
			...
			let data = {
				sender: this.id,
				receiver: this.ajax.receiver,
				content: this.content,
				type:0,
				time:that.$u.nowDateTime()
			};
			this.socket.send({
